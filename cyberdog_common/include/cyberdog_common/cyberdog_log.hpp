...
({\
  ...
  if(grade > 0)\
  {\
    nowmsg << "\n\t○ The trigger of the \33[1m\33[3m" << __FUNCTION__ << "()\33[23m\33[22m function in the \33[1m\33[4m" << __FILE__ <<"\33[24m\33[22m file of the source code is located at line \33[1m" << __LINE__ << "\33[22m";\
    if(grade > 1)\
    {\
      const char* result;\
      ...
      {\
        nowmsg << "\n\t◎ The \33[1m\33[4m" << process_path << "\33[24m\33[22m executable file compiled from \33[1m\33[4m" << __DATE__ << " - " << __TIME__ << "\33[24m\33[22m";\
        if(grade > 2)\
        {\
          ...
          {\
            ...
            nowmsg  << "\n\t● The \33[1m" << process_name << "\33[22m process captures a message with a process ID of \33[1m" << process_id << "\33[22m and a parent process ID of \33[1m" << parent_process_id << "\33[22m";\
          }\
        }\
      }\
    }\
...
#define DEBUG_FUNCTION_STREAM(function, msg) RCLCPP_DEBUG_FUNCTION(get_logger(), function, GET_MSG(msg, 0))
#define INFO_FUNCTION_STREAM(function, msg) RCLCPP_INFO_FUNCTION(get_logger(), function, GET_MSG(msg, 0))
#define WARN_FUNCTION_STREAM(function, msg) RCLCPP_WARN_FUNCTION(get_logger(), function, GET_MSG(msg, 1))
#define ERROR_FUNCTION_STREAM(function, msg) RCLCPP_ERROR_FUNCTION(get_logger(), function, GET_MSG(msg, 2))
#define FATAL_FUNCTION_STREAM(function, msg) RCLCPP_FATAL_FUNCTION(get_logger(), function, GET_MSG(msg, 3))

#define DEBUG_EXPRESSION_STREAM(expression, msg) RCLCPP_DEBUG_EXPRESSION(get_logger(), expression, GET_MSG(msg, 0))
#define INFO_EXPRESSION_STREAM(expression, msg) RCLCPP_INFO_EXPRESSION(get_logger(), expression, GET_MSG(msg, 0))
#define WARN_EXPRESSION_STREAM(expression, msg) RCLCPP_WARN_EXPRESSION(get_logger(), expression, GET_MSG(msg, 1))
#define ERROR_EXPRESSION_STREAM(expression, msg) RCLCPP_ERROR_EXPRESSION(get_logger(), expression, GET_MSG(msg, 2))
#define FATAL_EXPRESSION_STREAM(expression, msg) RCLCPP_FATAL_EXPRESSION(get_logger(), expression, GET_MSG(msg, 3))
